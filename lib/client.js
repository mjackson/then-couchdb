...
function Client(options) {
  ...
  if (options.cache) {
    if (options.cache === true) {
      ...
      if (cache instanceof Cache) {
        ...
        throw new Error('Client cache must be an instance of couchdb.Cache');
      }
    }
  }
  ...
  if (options.database) {
    this.use(options.database);
  }
}

/**
 * Sets the name of the database this client uses for all database-level
 * requests.
 */
...
/**
 * Sets the cache for this client to use. The pruneInterval is used to
 * set the time at which the cache will automatically be pruned. Set to a
 * falsy value to disable.
 */
Client.prototype.useCache = function (cache, pruneInterval) {
  pruneInterval = pruneInterval || ONE_MINUTE;
  ...
  if (this.cachePruneTimer) {
    ...
    delete this.cachePruneTimer;
  }

  if (pruneInterval) {
    var self = this;
    this.cachePruneTimer = setInterval(function () {
      self.pruneCache();
    }, pruneInterval);
  }
};
...
/**
 * Tells this client to stop using the cache.
 */
Client.prototype.stopCaching = function () {
  delete this.cache;
  delete this.cacheHits;
...
/**
 * Gets a value from this client's cache. Gracefully recovers from errors by
 * logging them and returning undefined.
 */
Client.prototype.getCache = function (key) {
  var self = this;
  return when(this.cache.get(key), function (doc) {
    if (doc) self.cacheHits++;
    return doc;
  }, function (error) {
    console.log('Error getting value from cache: ' + error);
  });
};

/**
 * Sets a value in this client's cache. Gracefully recovers from errors by
 * logging them and returning undefined.
 */
Client.prototype.setCache = function (key, value) {
  return when(this.cache.set(key, value), undefined, function (error) {
    console.log('Error setting value in cache: ' + error);
  });
};

/**
 * Purges this client's cache. Gracefully recovers from errors by
 * logging them and returning undefined.
 */
Client.prototype.purgeCache = function (key) {
  return when(this.cache.purge(key), undefined, function (error) {
    console.log('Error purging cache: ' + error);
  });
};

/**
 * Prunes this client's cache. Gracefully recovers from errors by
 * logging them and returning undefined.
 */
Client.prototype.pruneCache = function () {
  return when(this.cache.prune(), undefined, function (error) {
    console.log('Error pruning cache: ' + error);
  });
};

// Returns a promise for the response to a request to the URL "root".
Client.prototype.rootRequest = function (options) {
  options = options || {};

  var httpModule = HTTP_MODULES[this.protocol];
  ...
  if (query && !isEmpty(query)) {
    params.path += '?' + qs.stringify(prepareQuery(query));
  }

  params.headers = utils.merge({}, this.defaultHeaders, options.headers || {});
  params.agent = HTTP_AGENTS[this.protocol];
...
Client.prototype.save = function (doc) {
  ...
  if (doc._id) {
    options.method = 'PUT';
    options.path = '/' + prepareKey(doc._id);
  } else {
    options.method = 'POST';
  }
  ...
  return this.request(options).then(getDoc).then(function (data) {
    ...
    if (self.cache && !doc._deleted) {
      return self.setCache(doc._id, doc).then(function () {
        return doc;
      });
    }
...
Client.prototype.bulkDocs = function (docs) {
  if (!docs || !docs.length) return when([]);
  ...
  return this.request(options).then(getDoc).then(function (results) {
    ...
    var updatedDocs = docs.map(function (doc, i) {
      ...
      if (result && !result.error) {
        ...
        doc._rev = result.rev;
        if (self.cache && !doc._deleted) {
          promises.push(self.setCache(doc._id, doc));
        }
      }
...
/**
 * A high-level function that returns a promise for the document with the given
 * key. Uses the cache, if present, to avoid a database call.
 */
Client.prototype.getKey = function (key) {
  if (!this.cache) return this.get(key);

  var self = this;
  return this.getCache(key).then(function (doc) {
    ...
    return self.get(key).then(function (doc) {
      ...
      return self.setCache(key, doc).then(function () {
        return doc;
      });
    });
  });
...
/**
 * A high-level function that returns a promise for an array of all documents
 * with the given keys. Uses the cache, if present, to avoid a database call.
 */
Client.prototype.getKeys = function (keys) {
  if (!keys || !keys.length) return when([]);
  if (!this.cache) return this.allDocs({ keys: keys });
  ...
  return when.all(promises).then(function (docs) {
    ...
    return self.allDocs({ keys: missingKeys }).then(function (missingDocs) {
      // Add the found documents to the cache.
      var foundDocs = missingDocs.filter(Boolean);
      var promises = foundDocs.map(function (doc) {
        return self.setCache(doc._id, doc);
      });
...
function makeAttachmentPath(key, name) {
  return '/' + prepareKey(key) + '/' + encodeURIComponent(name);
}
