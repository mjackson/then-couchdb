...
Client.prototype.rootRequest = function (options) {
  ...
  if (!isEmpty(options.query)) {
    params.path += '?' + qs.stringify(prepareQuery(options.query));
  }
...
Client.prototype.get = function (key, query) {
  ...
  return this.getCache(key).then(function (doc) {
    ...
    return getKey(self, key).then(function (doc) {
      if (!doc) return doc;
...
Client.prototype.getAll = function (keys) {
  ...
  return when.all(promises).then(function (docs) {
    ...
    return getKeys(self, missingKeys).then(function (missingDocs) {
