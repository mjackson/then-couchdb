...
// TODO: Inherit from EventEmitter and cause clients to emit events for errors
// and when streaming.
function Client(options) {
  ...
  this._uuidCache = [];
  this.uuidCacheSize = 100;
}

// TODO: Make an agent for HTTPS as well.
Client.globalAgent = new http.Agent({ maxSockets: 20 });

// Makes a request to the root server URL.
Client.prototype.rootRequest = function (options, successHandler) {
  ...
  if (!this.host) throw new Error('Missing host');

  var opts = {};
  opts.agent = Client.globalAgent;
  ...
  if (query && !isEmpty(query)) opts.path += '?' + qs.stringify(prepareQuery(query));
  opts.headers = utils.merge({}, this.headers, options.headers || {});

  // console.log(opts.method + ' ' + opts.path); // good for debugging

  var deferred = q.defer();

  var req = httpModule.request(opts, function (res) {
    if (isSuccessCode(res.statusCode)) {
      successHandler(req, res, deferred);
    } else {
      getDoc(res).then(function (data) {
        var error = new Error(data.error + ', ' + data.reason);
        error.error = data.error;
        error.statusCode = res.statusCode;
        error.requestLine = req.method + ' ' + req.path;
...
// Makes a request to the database URL.
Client.prototype.request = function (options, successHandler) {
  options = options || {};
  if (!this.name) throw new Error('You must set a database name first!');
  options.path = '/' + this.name + (options.path || '');
  return this.rootRequest(options, successHandler);
};

// Returns a promise for an array of `count` UUIDs.
...
Client.prototype.remove = function (doc, preserveFields) {
  if (preserveFields) {
    doc._deleted = true;
  ...
  options.path = '/' + prepareId(doc._id);
  options.headers = { 'If-Match': doc._rev };
